...
class AuthRefferController extends Controller{
    ...
    public function header(Request $request)
    {
        try{
            ...
            if(in_array($params['method'], config('clientAuth.exceptAuth'))){
            ...
            }elseif(isset($params['auth_token']) && !in_array($params['method'], config('clientAuth.exceptAuth')) && !in_array($params['method'], config('clientAuth.noLoginAuth'))) {
                ...
                if (is_null($checkInfo)
                    ...
                    $params['userinfo']=[
						...
						'extended_data' =>  isset($checkInfo['data'][0]['extended_data'])?$checkInfo['data'][0]['extended_data']:'',// 用户会话扩展数据
                    ];
            ...
            }elseif(in_array($params['method'], config('clientAuth.noLoginAuth'))){//未登录可访问
