...
class OrderPayComponnet implements OrderCreater
{
     ...
     * 1.判断订单是否需要生成支付单
     * 2.如果订单不需要支付则更新订单状态
     * 3.如果订单为已支付 线下订单则生成发货单
     * 4.如果订单为已支付（线上，线下）调用风控看板信息
     * @return bool
     */
    public function create(): bool
    {
        ...
        try{
            ...
            //支付宝一次性支付
            elseif ($this->payType == PayInc::AlipayOne){
                $param['paymentAmount'] = $this->orderZujin + $this->orderYajin;
                $param['paymentFenqi'] =0;
                //创建普通支付
                \App\Order\Modules\Repository\Pay\PayCreater::createPayment($param);
            }
        ...
        if(!$this->isPay){
            ...
            if($appid ==137 || $appid ==139 || $appid == 208 || $orderType == OrderStatus::orderMiniService){
                $schedule->OrderRisk();
            }
