...
class WithholdAdvanceThree implements ShortMessage {
    ...
    public function notify(){
        ...
        //判断 支付宝小程序租期小于10天不触发
        if($goodsInfo['zuqi_type'] == OrderStatus::ZUQI_TYPE_DAY && $orderInfo['appid'] ==36){
            if($goodsInfo['zuqi']<10){
                return false;
            }
        }
        ...
        $dataSms =[
            ...
            'orderNo'      => $instalmentInfo['order_no'],
            'goodsName'     => $goodsInfo['goods_name'],
            'zuJin'         => $instalmentInfo['amount'],
            'createTime'    => 3,
			'zhifuri'       => date('Y-m-d',strtotime( '+3 day' )),
            'payTime'       => $instalmentInfo['day']." 日",
            'zhifuLianjie'  => createShortUrl($zhifuLianjie),
            'serviceTel'    => config('tripartite.Customer_Service_Phone'),
			'pingtaiName'   => PingtaiConfig::getName($orderInfo['appid']),
