...
class OrderGiveback implements UnderLine {
    ...
    public function execute( ){
		try{
			...
			//还机单不处于待支付的订单不允许线下还机
			if( $orderGivebackInfo['status'] != OrderGivebackStatus::STATUS_DEAL_WAIT_PAY  && $orderGivebackInfo['status'] != OrderGivebackStatus::STATUS_AGED_FAIL){
                return true;
			...
			if($order_type == \App\Order\Modules\Inc\OrderStatus::orderMiniService ){
                //查询芝麻订单
                $miniOrderInfo = \App\Order\Modules\Repository\OrderMiniRepository::getMiniOrderInfo($this->orderNo);
                \App\Lib\Common\LogApi::debug("[huanji-xianxiazhifu]查询芝麻订单",$miniOrderInfo);
                $mini_data = [
                    'out_order_no' => $miniOrderInfo['order_no'],//商户端订单号
                    'zm_order_no' => $miniOrderInfo['zm_order_no'],//芝麻订单号
                    'remark' => "异常处理",//订单操作说明
                    'out_trans_no'=>$miniOrderInfo['order_no'],//商户端交易号
                    'pay_amount'=>'00.00',//支付金额
                    'app_id' => $miniOrderInfo['app_id'],//小程序appid
                ];
                \App\Lib\Common\LogApi::info("[huanji-xianxiazhifu]通知芝麻取消请求参数",$mini_data);
                //通知芝麻取消请求
                $canceRequest = \App\Lib\Payment\mini\MiniApi::OrderClose($mini_data);
                if( !$canceRequest){
                    set_apistatus(ApiStatus::CODE_92700, '通知芝麻取消请求失败!');
                    \App\Lib\Common\LogApi::debug('[huanji-xianxiazhifu]通知芝麻取消请求失败');
                    throw new \Exception("通知芝麻取消请求失败");
                }
            }
